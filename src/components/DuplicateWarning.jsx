...
/**
 * mode="person"   — показывает совпадения персон (full_name, birth_year, region)
 * mode="document" — показывает совпадения документов (filename, similarity_score)
 */
      ...
      <div className="card max-w-lg w-full p-7 shadow-card-lg animate-slide-up">
        ...
        <div className="mb-5">
          ...
          <p className="text-sm text-slate-500 leading-relaxed">
            {mode === 'document'
              ? 'В архиве уже есть документы с похожим содержанием. Всё равно загрузить?'
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
          {items.map(item => (
            <div key={item.id} className="flex items-center gap-3 p-3.5 rounded-lg bg-slate-50 border border-slate-200">
              <div className="flex-1 min-w-0">
                {mode === 'document' ? (
                  <>
                    <p className="font-serif font-semibold text-sm text-slate-800 truncate">📄 {item.filename}</p>
                  </>
                ) : (
                  <>
              ...
              <div className="text-right shrink-0">
                ...
                <p className={`text-sm font-bold ${
                  item.similarity_score > 0.8 ? 'text-red-600' : 'text-amber-600'
                }`}>
